...
CREATE DATABASE LOJAMARIACJ3019616;
GO


-- Habilita o contexto do banco de dados
USE LOJAMARIACJ3019616;
GO
...
SELECT	CodigoVendedor	AS 'Código Vendedor',
		NomeVendedor	AS 'Nome Vendedor',
		Salario			AS 'Salário'
		FROM VENDEDORES
		WHERE 
...
SELECT 
		ITEM_DE_PEDIDO.NumeroPedido					 AS 'Numero do Pedido',
		COUNT(DISTINCT ITEM_DE_PEDIDO.CodigoProduto) AS 'Total dos Produtos Diferentes',
		SUM(ITEM_DE_PEDIDO.Quantidade)				 AS 'Total de Itens'
		FROM 
		ITEM_DE_PEDIDO
		GROUP BY 
		ITEM_DE_PEDIDO.NumeroPedido;
GO
...
SELECT  CodigoVendedor	AS 'Código do Vendedor',
		NomeVendedor	AS 'Nome Vendedor',
		Salario			AS 'Salário',
		(SELECT AVG(Salario) FROM VENDEDORES) AS 'Media Salarial'
		FROM VENDEDORES
...
DROP DATABASE LOJAMARIACJ3019616;
GO